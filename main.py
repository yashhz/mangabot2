...
from database import init_db, ManhwaDB
...
Bot will automatically check for updates every 6 hours.
        """
    ...
    async def cmd_list_manhwa(self, message: Message):
        ...
        if not manhwa_list:
            await message.answer("No manhwa being tracked.")
            return
...
Status: Running ✅
        ...
        for manhwa in manhwa_list:
            try:
                ...
                for chapter in new_chapters:
                    # Process and deliver chapter
                    success = await self.process_and_deliver_chapter(manhwa, chapter)
                    if success:
    ...
    async def process_and_deliver_chapter(self, manhwa, chapter):
        ...
        try:
            ...
            await self.send_chapter_to_channel(pdf_path, manhwa.name, chapter['name'])
    ...
    async def send_chapter_to_channel(self, pdf_path, manhwa_name, chapter_name):
        ...
        try:
            ...
            logger.info(f"Sent {chapter_name} to channel")
        except Exception as e:
